...
do 
        ...
        do
		rosbag record -q -b 2048 --duration=300 -O "$FILEINDEX" -e "/actuator_states" "/cmd_vel" "/controllers/(.*)" "/dbus_data" "/detection" "/joint_states" "/odom" "/rm_referee/(.*)" "/power_management/(.*)" "/rm_hw/(.*)" "/rm_track/(.*)" "/tf" "/tf_static" "/manual_to_referee" "/track" "/rosout" "/cmd_chassis" "/state"
